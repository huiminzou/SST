...
#HARDCODES
num_of_panels=3
mth=11#month of climatology
day=16#day of climatology
temprange=np.arange(7,12,0.05)
colorticks=np.linspace(7,12,6)
time_wanted=[dt.datetime(2017,11,11,23,59,59,0),dt.datetime(2017,11,23,23,59,59,0),dt.datetime(2017,mth,day,0,0,0)]
...
fig,axes=plt.subplots(num_of_panels,1,figsize=(7,11))
...
for i in range(num_of_panels):
    ...
    if i==num_of_panels-1:
        ...
        url1=url[i]+'?lat[0:1:4499],lon[0:1:4999],'+'sst['+str(index)+':1:'+str(index)+'][0:1:4499][0:1:4999]'+',day['+str(index)+':1:'+str(index)+']'
    else:
    ...
    if i==2:
        t=axes[2].set_title(str(time_wanted[i].strftime("%d-%b"))+'-clim',loc='center')
    else:
